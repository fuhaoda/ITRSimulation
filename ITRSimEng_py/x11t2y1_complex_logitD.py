...
NUMBER_RESPONSE = 2
Y_DIMENSION = 1
OUTPUT_PREFIX = "001_x11t2y1_complex_logitD"
...
def y_func(x, a, ydim, dg):
    ...
    """
    z = np.log(np.log( ((x[:,6]==3).reshape(-1,1) + (x[:,7]==3).reshape(-1,1) + \
            5*((x[:,5]==2).reshape(-1,1) + np.logical_and(x[:,0]==1, x[:,1]==1).reshape(-1,1)) * (2*a-3) + 20)**2))
