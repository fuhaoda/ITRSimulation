...
NUMBER_ACT = 2
Y_DIMENSION = 1
OUTPUT_PREFIX = "x10t3y1_linear_circle"
...
def y_func(x, a, ydim,dg):
    ...
    Y = 1-2X1 + X2 -X3 + I(X1^2+X2^2+X3^2<1.5)I(A=1)+I(1.5=<X1^2+X2^2+X3^2<2.5)I(A=2)+I(X1^2+X2^2+X3^2>=2.5)I(A=3)
    
    """
    m = np.array(x).shape[0]
    y = 1 - np.multiply(2,x[:, [0]])+ x[:, [1]] - x[:, [2]] +         np.multiply(x[:, [0]]**2+ x[:, [1]]**2 + x[:, [2]]**2<1.5, a==1)+        np.multiply(np.logical_or(x[:, [0]]**2+ x[:, [1]]**2 + x[:, [2]]**2<2.5,x[:, [0]]**2+ x[:, [1]]**2+ x[:, [2]]**2>=1.5), a==2)+        np.multiply(x[:, [0]]**2+ x[:, [1]]**2 + x[:, [2]]**2>=2.5, a==3)+        + dg.randn(m, ydim)
    return y
