...
NUMBER_RESPONSE = 2
Y_DIMENSION = 1
OUTPUT_PREFIX = "001_x4t2y1_discrete_linearF"
...
class CaseDataGenerator(DataGenerator):
    ...
    def exponential(self, scale=1, size=None):
        return self.state.exponential(scale, size)
...
def y_func(x, a, ydim, dg):
    ...
    y = 2 + 2*(a-1) + 2*z1 +2*z2 + 2*(a-1)*z1 + 2*(a-1)*z2 + dg.exponential(scale=1,size=(x.shape[0],1))
